...
<div class="container-fluid vw-100 vh-100 row align-items-center d-flex flex-column">
    ...
    <div class="form-container row-5 col-4">
        <form action="{{ route('recipe_groups.update', [Auth::user()->user_id, $recipe_group->recipe_group_id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <label for="name">Введіть нову назву Групи Рецептів</label>
            <input type="text" id="name" name="name" placeholder="Назва" value="{{ $recipe_group->name }}">
